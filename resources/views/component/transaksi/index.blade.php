...
<div class="container-fluid px-4">
    ...
    <div class="d-flex">
        <a type="button" class="btn btn-info mb-3" href="{{ route('sale.exportExcel') }}">Export Penjualan (.xlsx)</a>
                ...
                <tbody >
                    ...
                    @foreach ($sales as $item)
                        <tr class="d-flex align-items-center">
                            <td>{{ $no++ }}</td>
                            <td>{{ $item->customers->customer_name ?? 'NON-MEMBER' }}</td>
                            ...
                            <td class="me-4 mb-4 d-flex ms-auto">
                                ...
                                <a type="button" class="btn btn-primary ms-1" href="{{ route('sale.exportPDF', $item->id) }}">
                                    Unduh Bukti
                                </a>
                            </td>
                        </tr>
                                ...
                                <div class="modal-content p-2 rounded-0 shadow">
                                    ...
                                    <div class="mb-3 small text-muted">
                                        ...
                                        <div>
                                            ...
                                            <div>No. HP: {{ $lihat->customers ? $lihat->customers->no_telp : '-' }}</div>
                                            <div>Poin Member:  {{ $lihat->customers ? $lihat->customers->poin : 0 }}</div>
                                        </div>
                                        <div>
                                            <div>Bergabung Sejak: {{ $lihat->customers ? $lihat->customers->created_at->format('d F Y') : '-' }}</div>
                                        </div>
                                        </div>
                                    </div>
                                    ...
                                    <table class="table table-borderless mb-0">
                                        ...
                                        @foreach ($detail_sales as $jual)
                                            @if ($lihat->id == $jual->sale_id)
                                                <tr>
                                                    <td> {{ $jual->products ? $jual->products->name_product : '-' }} </td>
                                                    <td class="text-center">{{ $jual->quantity }}</td>
                                                    <td class="text-end"> {{ 'Rp. ' . number_format($jual->products['price'], 0, ',', '.') }} </td>
